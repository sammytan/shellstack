...
local ok,error = pcall(function()
    ...
    if ngx.var.cookie_server_name_session==nil and ngx.ctx.ip~=nil and ngx.ctx.ua~=nil and ngx.ctx.server_name~=nil and Config["session_token"]~=nil then 
        local session_value=ngx.md5(ngx.ctx.ip..ngx.ctx.ua..ngx.ctx.server_name..Config["session_token"])
        local new_cookie="server_name_session="..session_value.."; Max-Age=86400; httponly; path=/"
        if ngx.header.Set_Cookie ==nil then 
            ngx.header.Set_Cookie =new_cookie
        else
            local existing_cookies = ngx.header.Set_Cookie or {}
            if type(existing_cookies) == "string" then
                existing_cookies = { existing_cookies }
            end
            table.insert(existing_cookies, new_cookie)
            ngx.header.Set_Cookie = existing_cookies
        end
    end
	return ingos
end)
...
if not ok then
    ...
    if not ngx.shared.spider:get("btwaf_header") then 
        Public.logs(error)
        ngx.shared.spider:set("btwaf_header",1,180)
    end 
end 
