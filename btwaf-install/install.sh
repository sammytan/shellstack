...
{ 
	...
	# ---------------------------------------------------------------------------
	# ShellStack Redis 整页缓存（官方包植入，维护说明见 SHELLSTACK_CACHE_IMPL.md）
	# 与 btwaf-ext/README.md 一致，包含：lib/cache.lua、shellstack_cache_config、body/init/waf/header 钩子
	# - lib：cache.lua、shellstack_cache_config.lua（配置读面板 JSON）
	# - overlay/body.lua：eof 时 schedule_body_page_cache（否则 Redis 无写入）
	# - init.lua：cache = require "cache"（预加载模块）
	# - waf.lua 末尾：btwaf_run 成功后再 try_access_cache_hit
	# - header.lua 末尾：apply_header_filter_headers（X-Shellstack-* 等）
	# - 首次下发 shellstack_cache_config.json
	# ---------------------------------------------------------------------------
	if [ -d "$pluginPath/shellstack_impl" ]; then
		\cp -a -f "$pluginPath/shellstack_impl/lib/cache.lua" "$btwaf_path/lib/cache.lua"
		\cp -a -f "$pluginPath/shellstack_impl/lib/shellstack_cache_config.lua" "$btwaf_path/lib/shellstack_cache_config.lua"
		if [ -f "$pluginPath/shellstack_impl/overlay/body.lua" ]; then
			\cp -a -f "$pluginPath/shellstack_impl/overlay/body.lua" "$btwaf_path/body.lua"
		fi
		if ! grep -q 'cache = require "cache"' "$btwaf_path/init.lua" 2>/dev/null; then
			awk '/Json = require "cjson"/ && !done {
				print
				print "cache = require \"cache\"  -- ShellStack 页缓存（SHELLSTACK_CACHE_IMPL.md）"
				done=1
				next
			}
			{ print }' "$btwaf_path/init.lua" > "${btwaf_path}/init.lua.shellstack.$$" && mv "${btwaf_path}/init.lua.shellstack.$$" "$btwaf_path/init.lua"
		fi
		if [ ! -f "$pluginPath/shellstack_cache_config.json" ] && [ -f "$pluginPath/shellstack_impl/shellstack_cache_config.json" ]; then
			\cp -a -f "$pluginPath/shellstack_impl/shellstack_cache_config.json" "$pluginPath/shellstack_cache_config.json"
			chown www:www "$pluginPath/shellstack_cache_config.json" 2>/dev/null || true
		fi
		if ! grep -q 'shellstack_header_cache_hook' "$btwaf_path/header.lua" 2>/dev/null; then
			cat >> "$btwaf_path/header.lua" << 'SHELLSTACK_HDR_EOF'

-- shellstack_header_cache_hook（SHELLSTACK_CACHE_IMPL.md；header_filter 写 X-Shellstack-*）
do
	local _ok, _c = pcall(require, "cache")
	if _ok and _c and type(_c.apply_header_filter_headers) == "function" then
		_c.apply_header_filter_headers()
	end
end
SHELLSTACK_HDR_EOF
		fi
		if ! grep -q 'shellstack_page_cache_after_btwaf' "$btwaf_path/waf.lua" 2>/dev/null; then
			cat >> "$btwaf_path/waf.lua" << 'SHELLSTACK_WAF_EOF'

-- [ShellStack] 整页 Redis 缓存：仅在 btwaf_run 成功（ok 为 true）后执行，避免绕过 WAF。
-- shellstack_page_cache_after_btwaf（install.sh 幂等标记，勿删）
if ok then
	do
		local _ok, _c = pcall(require, "cache")
		if not _ok then
