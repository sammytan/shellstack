...
# WAF防火墙 敏感词过滤
#----------------------
# ShellStack：本文件由 btwaf-ext/btwaf/body.lua 覆盖官方逻辑（install.sh 复制自 shellstack_impl/overlay/body.lua）
# 须在 eof 时调用 cache.schedule_body_page_cache，否则 Redis 只读不写、access 永远 MISS（见 README.md）
#----------------------
]]--
    ...
    if eof then
        ...
        do
            ...
            if okc and c and type(c.schedule_body_page_cache) == "function" then
                -- nil 表示使用 shellstack_cache_config.page_ttl_seconds（与面板 JSON 一致）
                c.schedule_body_page_cache(nil, whole)
            end
        end
    end
end

-- 让 Nginx body_filter 阶段自动执行
