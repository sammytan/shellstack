...
download_module_dependencies() {
  ...
  local include_files=(
    ...
    "modsecurity_ipinfo.sh"
    "modsecurity_git_clone.sh"
    "modsecurity_git_submodules.sh"
    "modsecurity.sh"
    "geoip.sh"
...
show_help() {
  ...
  echo "  # 仅 exporter + Consul，并强制重跑 exporter（--force 在此组合下不触发 ModSecurity/宝塔主安装）"
  echo "  curl -fsSL ${BASE_URL}/shellstack.sh | sudo bash -s modsecurity --with-exporter --with-consul-token --force"
  echo "  # ModSecurity/宝塔「全套」强制 + exporter（须带主安装参数，例如 --deploy-conf；不要依赖「仅 exporter + --force」）"
  echo "  curl -fsSL ${BASE_URL}/shellstack.sh | sudo bash -s modsecurity --with-exporter --with-consul-token --deploy-conf --force"
  ...
  echo "  # Token 建议用环境变量（避免进历史记录）："
  echo "  export CONSUL_HTTP_TOKEN=YOUR_TOKEN"
  echo "  curl -fsSL ${BASE_URL}/shellstack.sh | sudo bash -s modsecurity --with-exporter=http://127.0.0.1:8500"
  echo "  # 仅同步 ModSecurity 自定义规则文件 custom_modsec_rules.conf（从站点拉取正文，不重编 Nginx / 不部署 CRS）"
  echo "  curl -fsSL ${BASE_URL}/shellstack.sh | sudo bash -s modsecurity --update-modesc-conf"
  echo "  # 错误写法：curl ... | bash shellstack.sh ...（会从当前目录读本地 shellstack.sh，管道脚本被忽略）"
  echo ""
  echo "  # 本地使用"
