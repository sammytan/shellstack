...
选项:
  ...
  说明: 与 ModSecurity/宝塔 安装类参数（如 --version、--deploy-conf、--bt-openresty、--force 等）**同时未出现**时，--with-exporter / --with-consul-token 触发 **exporter 独立流程**（不跑主安装）。若要在同一条命令里先完整安装 ModSecurity 再注册 exporter，须带上至少一个安装类参数。EXPORTER_CONSUL_ADDR 与 EXPORTER_PROMETHEUS_SERVER（已弃用）等价。
