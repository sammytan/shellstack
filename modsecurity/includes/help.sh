...
选项:
  ...
  说明: --extend-btwaf-cache 环境变量：SHELLSTACK_BTWAF_PANEL_INSTALL=0 跳过面板 install.sh；已安装 BTwaf（/www/server/btwaf/socket 存在）默认不重复 install；SHELLSTACK_BTWAF_FORCE_PANEL_INSTALL=1 强制重装；SHELLSTACK_BTWAF_OVERLAY_SRC=目录 指定本地扩展；SHELLSTACK_BTWAF_OVERLAY_BASE_URL=URL 覆盖 HTTP 根；SHELLSTACK_BTWAF_CACHE_LUA_URL=单文件 仅拉 cache.lua；SHELLSTACK_BTWAF_OFFICIAL_REF=目录 指定官方参考树（含 resty/redis.lua），用于补齐 /www/server/btwaf/lib/resty/redis.lua（与 init.lua 的 package.path 一致；不下载）；SHELLSTACK_INSTALL_REDIS=0 跳过 Redis；SHELLSTACK_REDIS_VER 可填宝塔版本号（如 8.0.5/8.2.3/8.4.0，传 8.0/8.2 会自动映射）；页缓存 Lua 连接 Redis 默认 127.0.0.1:6379，可设 SHELLSTACK_REDIS_HOST / SHELLSTACK_REDIS_PORT / SHELLSTACK_REDIS_DB（nginx 主配置须 env 同名声明）；整页缓存使用**单个 Redis Hash**（默认键 btwaf_cms_cache），field=md5(server_name|uri|args|User-Agent)（与官方 WAF 无 UA 时用 btwaf_null）；SHELLSTACK_CACHE_VARY_UA=0 则 field 不含 UA；TTL 在 JSON 的 expires_at；可设 SHELLSTACK_CACHE_HASH_KEY 覆盖 Hash 名；SHELLSTACK_BTWAF_LEGACY_TARBALL=1 旧版全量 tar；SHELLSTACK_BTWAF_OVERLAY_INIT_LUA=1 覆盖 init.lua。
