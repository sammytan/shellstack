...
选项:
  ...
  --update-modesc-conf   宝塔：仅从站点 wget/curl 拉取 custom_modsec_rules 正文（默认 URL: \$SHELLSTACK_BASE_URL/modsecurity/includes/custom_modsec_rules.conf，可用 CUSTOM_MODSEC_RULES_STATIC_URL 覆盖），再与本机检测到的 GeoIP 库路径拼接后写入 custom_modsec_rules.conf；成功后执行 \$BT_NGINX_BIN -t，通过则 **/etc/init.d/nginx restart**。**不重编 Nginx 二进制**、**不部署 CRS**。仅需已安装宝塔面板（不要求 BTwaf）。
  ...
  说明: --deploy-conf 从 ModSecurity 仓库复制 modsecurity.conf-recommended / unicode.mapping；若 git 失败会回退从 raw.githubusercontent.com/owasp-modsecurity/ModSecurity 下载（MODSECURITY_CONF_SAMPLES_TAG 默认 v3.0.10）。custom_modsec_rules.conf 的正文与 --update-modesc-conf 相同：默认从 \$SHELLSTACK_BASE_URL/modsecurity/includes/custom_modsec_rules.conf 下载；离线或失败时使用随脚本自带的 includes/custom_modsec_rules.conf。
