...
选项:
  ...
  --update-modesc-conf   宝塔：仅从站点 wget/curl 拉取 custom_modsec_rules 正文（默认 URL: \$SHELLSTACK_BASE_URL/modsecurity/includes/custom_modsec_rules.conf，可用 CUSTOM_MODSEC_RULES_STATIC_URL 覆盖），再与本机检测到的 GeoIP 库路径拼接后写入 custom_modsec_rules.conf；**不重编 Nginx**、**不部署 CRS**。仅需已安装宝塔面板（不要求 BTwaf）。
