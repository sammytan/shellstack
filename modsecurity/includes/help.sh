...
选项:
  ...
  说明: --with-exporter 会尝试通过包管理器安装 node_exporter（默认端口 9100，可设 EXPORTER_LISTEN_PORT），配置 textfile 采集（每分钟）：shellstack_process_up（nginx/mysql/redis/宝塔面板）、shellstack_php_fpm_up（扫描 /www/server/php/*/）、shellstack_nginx_stub_*；并向 Consul 注册。stub 默认探测常见 URL，失败且为宝塔 /www/server/nginx 时自动在 nginx.conf 的 http{} 内 include shellstack_stub_status.conf（127.0.0.1:8899/nginx_stub_status），并写进 cron 的 SHELLSTACK_NGINX_STUB_URLS；关闭注入设 SHELLSTACK_NGINX_STUB_INJECT=0，改端口设 SHELLSTACK_NGINX_STUB_LISTEN_PORT。常规系统指标为 node_exporter 默认（node_*）。**root 且未设 SHELLSTACK_EXPORTER_SKIP_FIREWALL=1 时**，会按本机环境依次尝试 **firewalld → ufw → iptables（含 iptables-nft / iptables-legacy）→ nft** 放行 metrics 端口：**默认仅允许 CONSUL_HTTP_ADDR 解析出的 Consul 主机 IPv4**；多源或 CIDR 用 EXPORTER_METRICS_ALLOW_FROM（逗号分隔）；关闭自动配防火墙用 SHELLSTACK_EXPORTER_SKIP_FIREWALL=1。**root 且未设 SHELLSTACK_EXPORTER_SKIP_HOSTNAME=1** 时，在安装 node_exporter 前将静态主机名设为「二位地区码-公网IPv4」（如 HK-1.2.3.4）；公网 IP 依次轮询 ipify、ipinfo.io/ip、ifconfig.me、icanhazip、ident.me、AWS checkip 等，地区码由 ip-api.com 与 ipinfo.io 互补；可设 SHELLSTACK_EXPORTER_PUBLIC_IP / SHELLSTACK_EXPORTER_GEO_CODE / SHELLSTACK_EXPORTER_GEO_HOSTNAME 覆盖。Consul 注册默认带扩展 **Tags/Meta**（主机名、系统、是否宝塔、textfile 路径与是否注入 node_exporter 等）；额外标签/元数据用 CONSUL_SERVICE_TAGS、CONSUL_SERVICE_META。/metrics 中 smartmon_ 等前缀来自本机其它 node_exporter 采集器；shellstack_ 前缀来自 textfile（若缺失请查 node_exporter 是否带 --collector.textfile.directory 及 /var/log/shellstack-node-exporter-textfile.log）。Consul 默认见 exporter.sh（自动 export + /etc/profile.d/shellstack-consul-env.sh）；SHELLSTACK_EXPORTER_NO_BUILTIN_TOKEN=1、SHELLSTACK_EXPORTER_SKIP_PERSIST_ENV=1 见 exporter 头注释。
  说明: 与 ModSecurity/宝塔 安装类参数（如 --version、--deploy-conf、--bt-openresty、--force 等）**同时未出现**时，--with-exporter / --with-consul-token 触发 **exporter 独立流程**（不跑主安装）。**--with-exporter 与 --with-consul-token 可省略 = 后的值**（与写全 URL/Token 一样走内置默认，见 exporter.sh）。**一旦加上 --force（或其它上述安装类参数）**，即视为要跑完整主流程，不再进入「仅 exporter」模式——这与是否省略 Consul 地址无关。若要在同一条命令里先完整安装 ModSecurity 再注册 exporter，须带上至少一个安装类参数。EXPORTER_CONSUL_ADDR 与 EXPORTER_PROMETHEUS_SERVER（已弃用）等价。
...
示例:
  ...
  # 远程：强制覆盖安装（--force）并同时注册 exporter；Consul 地址与 Token 可改为你的环境，Token 建议用环境变量避免进 shell 历史
  curl -fsSL https://shellstack.example/shellstack.sh | sudo bash -s modsecurity --with-exporter=http://CONSUL:8500 --with-consul-token=secret --force
