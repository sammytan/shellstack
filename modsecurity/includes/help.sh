...
选项:
  ...
  --with-exporter[=ADDR] 安装 node exporter，并通过 Consul Agent API 注册服务（ADDR 可省略，省略时用 exporter.sh 内置默认 Consul；Prometheus 使用 consul_sd_configs 发现 target）
