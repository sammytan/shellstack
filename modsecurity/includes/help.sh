...
选项:
  ...
  说明: \`/var/log/shellstack-node-exporter-textfile.log\` 只承接 **cron 执行 textfile 脚本** 时的标准输出/错误；指标写入 \`.prom\` 文件而非该日志，故旧版脚本在一切正常时此文件可为空。**node_exporter（prometheus-node-exporter）守护进程** 的日志通常在 \`journalctl -u prometheus-node-exporter -e\`，不是该路径。
